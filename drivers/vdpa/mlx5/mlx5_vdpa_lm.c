			   ...
			   uint64_t log_size)
{
	...
	mr->mkey = mlx5_devx_cmd_mkey_create(priv->ctx, &mkey_attr, NULL, 0);
	if (!mr->mkey) {
...
mlx5_vdpa_lm_log(struct mlx5_vdpa_priv *priv)
{
	...
	if (RTE_VHOST_NEED_LOG(features)) {
		SLIST_FOREACH(virtq, &priv->virtq_list, next) {
			ret = mlx5_vdpa_virtq_state_modify(virtq, 0);
			if (ret)
				return -1;
			...
			size = (virtq->umems[0].size - priv->caps.umems[0].b) /
							  priv->caps.umems[0].a;
			rte_vhost_log_used_vring(priv->vid, virtq->index, 0,
						 MLX5_VDPA_USED_RING_LEN(size));
		}
	}
	return 0;
}
