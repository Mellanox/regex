 ...
 * @param[in] klm_array
 *   KLMs of indirect mkey.
 * @param[in] klm_num
 *   Number of KLM entries in the array.
 *   0 means to create a direct mkey, otherwise, to create an indirect mkey.
 *
 * @return
...
mlx5_devx_cmd_mkey_create(struct ibv_context *ctx,
			  ...
			  struct mlx5_klm *klm_array, int klm_num)
{
			     ...
			     struct mlx5_hca_attr *attr)
{
	...
	if (attr->vdpa.valid) {
		...
		if (rc || status) {
			...
			attr->vdpa.umems[0].a = MLX5_GET(virtio_emulation_cap,
							 hcattr,
							 umem_1_buffer_param_a);
			attr->vdpa.umems[0].b = MLX5_GET(virtio_emulation_cap,
							 hcattr,
							 umem_1_buffer_param_b);
			attr->vdpa.umems[1].a = MLX5_GET(virtio_emulation_cap,
							 hcattr,
							 umem_2_buffer_param_a);
			attr->vdpa.umems[1].b = MLX5_GET(virtio_emulation_cap,
							 hcattr,
							 umem_2_buffer_param_b);
			attr->vdpa.umems[2].a = MLX5_GET(virtio_emulation_cap,
							 hcattr,
							 umem_3_buffer_param_a);
			attr->vdpa.umems[2].b = MLX5_GET(virtio_emulation_cap,
							 hcattr,
