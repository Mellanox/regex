...
mlx5_rx_addr2mr_bh(struct mlx5_rxq_data *rxq, uintptr_t addr)
{
	...
	return mlx5_mr_addr2mr_bh(priv->sh->pd, ETH_DEV(priv)->data->port_id,
			 ...
			 unsigned mem_idx __rte_unused)
{
	...
	mlx5_mr_addr2mr_bh(sh->pd, dev->data->port_id, &sh->share_cache,
	       ...
	       uint64_t iova __rte_unused, size_t len __rte_unused)
{
	 ...
	 */
	++sh->share_cache.dev_gen;
	DEBUG("broadcasting local cache flush, gen=%d",	sh->share_cache.dev_gen);
	rte_smp_wmb();
	rte_rwlock_read_unlock(&sh->share_cache.rwlock);
	return 0;
}
		     ...
		     unsigned mem_idx __rte_unused)
{
	...
	lkey = mlx5_mr_addr2mr_bh(priv->sh->pd, dev->data->port_id,
