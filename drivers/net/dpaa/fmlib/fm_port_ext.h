...
typedef struct ioc_fm_pcd_prs_additional_hdr_params_t {
	...
	ioc_fm_pcd_hdr_prs_opts_u prs_opts;/**< A union according to header type,
...
typedef struct ioc_fm_pcd_prs_start_t {
	uint8_t	parsing_offset; /**< Number of bytes from beginning of packet to
				start parsing */
	ioc_net_header_type first_prs_hdr;/**< The type of the first header expected at
				'parsing_offset' */
...
/**
 ...
 @Param[in]	ioc_fm_pcd_port_schemes_params_t	Schemes parameters structure
...
/**
 ...
 @Param[in]	h_FmPort	A handle to a FM Port module.
 @Param[in]	syncReq	TRUE to request synchronization, FALSE otherwise.
...
typedef struct t_FmPortDsarFilteringInfo {
	...
	bool	ipProtPassOnHit;
			/* when TRUE, miss in the table will cause the packet to be dropped,
			...
			the packet to a queue it will cause a wake interrupt.
