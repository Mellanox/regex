...
static inline uint32_t
job_id_get(uint32_t qid, struct mlx5_regex_sq *sq) {
	return qid*sq_size_get(sq) + sq->pi%sq_size_get(sq);
}
		      ...
		      struct rte_regex_ops **ops, uint16_t nb_ops)
{
	...
	while ((cqe = poll_one(cq))) {
		...
		while (sq->ci != wq_counter) {
			...
			uint32_t job_id = job_id_get(sqid, sq);
