...
mlnx_update_database(struct mlx5_regex_priv *priv, uint8_t id)
{
	...
	for (i = 0; i < (MLX5_RXP_MAX_ENGINES + MLX5_RXP_SHADOW_EM_COUNT);
		...
		priv->db[i].db_assigned_to_eng_num = rxp_eng;
		db_free = i;
		break;
	}
                  ...
                  uint32_t len, uint8_t id)
{
	...
	/* Program RXP with the rules provided. */
        if (priv->prog_mode == MLX5_RXP_PRIVATE_PROG_MODE) {
        ...
        if (priv->prog_mode == RXP_SHARED_PROG_MODE) {
                ...
                ret = mlnx_write_shared_rules(priv, rules, rules->hdr.len, id,
                                              db_free);
                if (ret < 0) {
	...
	DRV_LOG(DEBUG, "max matches: %d, DDOS threshold: %d", reg >> 16,
        ...
        if ((reg >> 16) >= priv->nb_max_matches)
                ...
                ret = mlx5_devx_regex_register_write(ctx, id,
                                                     MLX5_RXP_CSR_MAX_MATCH,
                                                     (reg >> 16));
        ret |= mlx5_regex_register_write(ctx, id, RXP_CSR_MAX_PREFIX,
                                         (reg & 0xFFFF));
	ret |= mlx5_devx_regex_register_write(ctx, id,
                    ...
                    uint32_t count __rte_unused, uint8_t id)
{
        ...
        if (rules->hdr.cmd == MLX5_RXP_CTL_RULES_PGM) {
                ...
                ret = rxp_init_rtru(id, MLX5_RXP_RTRU_CSR_CTRL_INIT_MODE_L1_L2);
                if (ret < 0)
                        return ret;
        }
        if (rules->count == 0)
                ...
                if ((rules->rules[rule_cnt].type == MLX5_RXP_ROF_ENTRY_INST) ||
                    ...
                    (rules->rules[rule_cnt].type == MLX5_RXP_ROF_ENTRY_EM)) {
                        if ((rules->rules[rule_cnt].type ==
                            ...
                            (rxp_prog_mode == MLX5_RXP_SHARED_PROG_MODE)) {
 ...
 */
static int
mlx5_write_shared_rules(struct mlx5_regex_priv *priv,
...
rxp_db_setup(struct mlx5_regex_priv *priv)
{
        ...
        for (i = 0; i < (MLX5_RXP_MAX_ENGINES + MLX5_RXP_SHADOW_EM_COUNT); i++){
                ...
                /* Register the memory. */
                priv->db[i].umem = mlx5dv_devx_umem_reg(priv->ctx,
                                                        priv->db[i].ptr,
                                                        MLX5_MAX_DB_SIZE, 7);
                if (!priv->db[i].umem) {
...
tidyup_error:
        for (i = 0; i < (MLX5_RXP_MAX_ENGINES + MLX5_RXP_SHADOW_EM_COUNT); i++){
                ...
                if (priv->db[i].umem)
                        mlx5dv_devx_umem_dereg(priv->db[i].umem);
        }
        return -ret;
}
	...
	for (id = 0; id < MLX5_RXP_MAX_ENGINES; id++) {
		...
		if (ret) {
			DRV_LOG(ERR, "can't stop engine.");
                ...
                ret = program_rxp_rules(priv, cfg->rule_db, cfg->rule_db_len,
                                        id);
                if (ret < 0) {
		...
		if (ret) {
			DRV_LOG(ERR, "can't start engine.");
                ...
                DRV_LOG(ERR, "Regex import without valid rules set!");
                return -ENODEV;
        }
	return 0;
}
		     ...
		     const struct rte_regexdev_config *cfg)
{
	...
	if (cfg->rule_db != NULL) {
                ret = mlx5_regex_rules_db_import(dev, cfg);
                if (ret < 0) {
                        DRV_LOG(ERR, "Failed to program rxp rules.");
                        rte_errno = ENODEV;
